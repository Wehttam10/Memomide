    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <section className="panel">
          ...
          {topics.length > 0 ? (
            <div className="mt-4 space-y-3">
              {topics.map((topic) => (
          <div key={topic.id} className="rounded-lg border border-teal/15 bg-white/80 p-4 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <Link to={`/topics/${topic.id}`} className="font-bold hover:text-teal">{topic.title}</Link>
        ...
        <form onSubmit={addTopic} className="panel space-y-3 lg:sticky lg:top-24 lg:self-start">
          <h3 className="font-bold">Create topic</h3>
